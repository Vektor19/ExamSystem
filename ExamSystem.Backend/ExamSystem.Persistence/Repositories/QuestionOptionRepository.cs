...
namespace ExamSystem.Persistence.Repositories
{
    public class QuestionOptionRepository : IQuestionOptionRepository
    {
        ...
        public async Task<RepositoryOperationResult<IEnumerable<QuestionOption>>> GetAllAsync()
        {
            var questionOptions = await _dbContext.QuestionOptions.Include(qo => qo.Question)
                                                                  .Include(qo => qo.Answers)
                                                                  .ToListAsync();
            return RepositoryOperationResult<IEnumerable<QuestionOption>>.Ok(questionOptions);
        }
        public async Task<RepositoryOperationResult<QuestionOption>> GetByIdAsync(Guid id)
        {
            ...
            if (questionOption == null)
                return RepositoryOperationResult<QuestionOption>.Fail("Question option not found.");
            return RepositoryOperationResult<QuestionOption>.Ok(questionOption);
        }
